...
# Function to analyze A/B test results for Bounce Rate
def analyze_bounce_rate_test(file_path, start_date="2025-03-05", alpha=0.05):
    """
    Analyze A/B test results for Bounce Rate from an Excel file with date columns,
    ...
    try:
        ...
        print(f"Group A mean bounce rate: {group_a_mean:.2f}%, std: {group_a_std:.2f}%")
        print(f"Group B mean bounce rate: {group_b_mean:.2f}%, std: {group_b_std:.2f}%")

        # Calculate relative change (for bounce rate, a decrease is positive)
        rel_change = (group_b_mean - group_a_mean) / group_a_mean * 100
        improvement = rel_change < 0  # For bounce rate, negative change is an improvement
        ...
        # Interpret effect size (absolute value for bounce rate since decrease is positive)
        if abs(cohens_d) < 0.2:
        ...
        # Print results
        print("\n===== A/B Test Analysis Results for Bounce Rate =====")
        print(f"Analysis period: {min(date_columns).strftime('%Y-%m-%d')} to {max(date_columns).strftime('%Y-%m-%d')}")
        print(f"Baseline (Group A): {group_a_mean:.2f}%")
        print(f"Treatment (Group B): {group_b_mean:.2f}%")
        print(f"Absolute change: {group_b_mean - group_a_mean:.2f}%")
        print(f"Relative change: {rel_change:.2f}%")

        if improvement:
            print(f"Result: Bounce rate decreased by {abs(rel_change):.2f}% (improvement)")
        else:
            print(f"Result: Bounce rate increased by {abs(rel_change):.2f}% (not an improvement)")
        ...
        # Check for significance and business significance (5% change)
        if p_value < alpha:
            if improvement and abs(rel_change) >= 5:
                print("✅ RESULT: Statistically significant improvement with at least 5% reduction in bounce rate")
                print("   Null hypothesis rejected - The alternative hypothesis is supported")
            elif improvement:
                print("⚠️ RESULT: Statistically significant improvement but effect is less than 5% reduction")
                print(
                    "   Null hypothesis partially rejected - The improvement is statistically significant but does not meet business criteria")
            else:
                print("❌ RESULT: Statistically significant increase in bounce rate (negative result)")
                print("   Null hypothesis rejected - But the change is in the undesired direction")
        else:
            print("❓ RESULT: No statistically significant difference in bounce rate")
        ...
        plt.title('Daily Bounce Rate by Group')
        plt.legend()
        ...
        plt.title('Mean Bounce Rate with 95% CI')

        # Add percentage change annotation with appropriate color based on improvement
        ...
        change_text = f"{rel_change:.1f}% {'↓' if improvement else '↑'}"
        ...
        plt.tight_layout()
        plt.savefig('bounce_rate_results.png', dpi=300)
        print("Visualization saved as 'bounce_rate_results.png'")
...
results = analyze_bounce_rate_test('test_2_bounce_rate.xlsx', start_date="2025-03-05")